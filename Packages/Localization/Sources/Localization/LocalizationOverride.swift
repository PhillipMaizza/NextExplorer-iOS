...
public enum LocalizationOverride {
    ...
    public static func apply(_ language: String?) {
        ...
        overrideLocale = Locale(identifier: language)
    }

    /// Whether a language reads right to left, so the UI can flip `\.layoutDirection`. None of the
    /// currently shipped languages are RTL, but the plumbing is ready for one that is.
