...
struct ShakeEffectTests {
    ...
    func atRest() {
        let effect = DSShakeEffect(travelDistance: 10, numberOfShakes: 3, animatableData: 0)
    ...
    func fullCycleReturnsToRest() {
        ...
        let effect = DSShakeEffect(travelDistance: 16, numberOfShakes: 3, animatableData: 1)
        let transform = effect.effectValue(size: CGSize(width: 100, height: 40))
    ...
    func travelDistanceScalesTranslation() {
        let small = DSShakeEffect(travelDistance: 8, numberOfShakes: 3, animatableData: 0.5 / 3)
        let large = DSShakeEffect(travelDistance: 16, numberOfShakes: 3, animatableData: 0.5 / 3)
    ...
    func negativeInputMirrors() {
        let positive = DSShakeEffect(travelDistance: 10, numberOfShakes: 3, animatableData: 1.0 / 6)
        let negative = DSShakeEffect(travelDistance: 10, numberOfShakes: 3, animatableData: -1.0 / 6)
