...
public extension View {
    ...
    /// Login's labelled `DSFieldContainer` is the deliberate exception.
    func roundedFieldStyle(height: CGFloat = .size48, isFocused: Bool = false, bordered: Bool = true) -> some View {
        modifier(RoundedFieldModifier(height: height, isFocused: isFocused, bordered: bordered))
    }
}
