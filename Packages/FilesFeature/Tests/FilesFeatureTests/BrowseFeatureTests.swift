...
struct BrowseFeatureTests {
    ...
    @Test
    func editInOfficePresentsTheEditorAndAnEditedCloseRefetchesTheFolder() async {
        let serverURL = URL(string: "https://example.com")!
        ...
        let store = TestStore(initialState: state) {
            ...
            $0.filesClient.browse = { _, _ in
                BrowseResult(items: [item], access: FileAccess(canRead: true, canWrite: true, canUpload: false, canDelete: false, canShare: false, canDownload: true), path: "Docs")
            }
            $0.filesClient.favorites = { _ in [] }
        }
        store.exhaustivity = .off
        ...
        await store.send(.editInOfficeTapped(item)) {
            $0.officeEditor = OfficeEditorFeature.State(serverURL: serverURL, item: item, editor: .onlyOffice, mode: .edit)
        }

        await store.send(.officeEditor(.presented(.delegate(.closed(didEdit: true))))) {
            $0.officeEditor = nil
            $0.isLoading = true
        }
        await store.receive(\.itemsResponse)
    }

    @Test
    func editInOfficeIsANoOpWhenNoServerEditorIsEnabled() async {
        let serverURL = URL(string: "https://example.com")!
        let item = FileItem(name: "Report.docx", path: "Docs", dateModified: Date(), size: 10, kind: "docx")
        ...
        await store.send(.editInOfficeTapped(item))
        #expect(store.state.officeEditor == nil)
    }

    @Test
    func aReadOnlyItemStillOpensTheEditorInViewMode() async {
        let serverURL = URL(string: "https://example.com")!
        let item = FileItem(name: "Report.docx", path: "Docs", dateModified: Date(), size: 10, kind: "docx")
        var state = BrowseFeature.State(serverURL: serverURL, directoryPath: "Docs", title: "Docs")
        state.access = FileAccess(canRead: true, canWrite: false, canUpload: false, canDelete: false, canShare: false, canDownload: true)
        state.$serverFeatures.withLock {
            $0 = ServerFeatures(office: ServerFeatures.OfficeEditors(isCollaboraEnabled: true))
        }
        let store = TestStore(initialState: state) { BrowseFeature() }
        store.exhaustivity = .off
